...
namespace SDR
{
	namespace Sampler
	{
		EasyByteSampler::EasyByteSampler(
			...
			, m_Settings(settings)
			, m_Frame(Frame(settings.Height_get() * settings.Width_get()))
		{
			...
			m_LastSample = twoPoint ? std::make_unique<unsigned char[]>(height * pitch) : nullptr;
			m_PrintMem = std::make_unique<unsigned char[]>(height * pitch);
		}
		...
		void EasyByteSampler::PrintFrame()
		{
			...
			unsigned char * data = m_PrintMem.get();
			...
			m_FramePrinter->Print(m_PrintMem.get());
		}
		...
		void EasyByteSampler::Sample(unsigned char const * data, double time)
		{
			...
			{
				memcpy(m_LastSample.get(), data, m_Settings.Height_get() * m_Pitch * sizeof(unsigned char));
		...
		void EasyByteSampler::ScaleFrame(float factor)
		{
			...
			if (0 == w * factor)
			{
				...
				memset(m_Frame.Data.get(), 0, m_Settings.Height_get() * m_Settings.Width_get() * sizeof(float));
				return;
			}
