...
| args | Extra arguments to provide the start with, this works the same as the Steam launch parameters with one difference: a `-game` parameter must be specified here which contains the short name of the game. <br> This is usually `cstrike`, `csgo`, `tf`, `momentum` or similar.
