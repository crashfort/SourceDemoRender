...
{
    // The buttons are embedded in the networked zoom variable.
    // Search for "m_iLastZoom", use offset parameter.

    u8* addr = (u8*)game_scan_pattern("client.dll", "41 B8 ?? ?? ?? ?? 89 7C 24 20 48 8D 15 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 44 8D 4F 01 48 89 7C 24 ?? 41 B8 ?? ?? ?? ?? 89 7C 24 20 48 8D 15 ?? ?? ?? ?? 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 05 ?? ?? ?? ?? 41 B8 ?? ?? ?? ?? 48 89 44 24 ?? 44 8D 4F 0C 48 8D 15 ?? ?? ?? ?? 89 7C 24 20 48 8D 0D ?? ?? ?? ?? E8 ?? ?? ?? ?? 48 8D 05", NULL);
