...
The default video encoder is ``libx264``. Other available is ``libx264rgb`` which will produce an RGB video with no color loss. Note however that ``libx264rgb`` encodes slower than ``libx264`` and will greatly increase file size. Note that some video editors are not capable of opening libx264rgb encoded videos or MKV containers.
