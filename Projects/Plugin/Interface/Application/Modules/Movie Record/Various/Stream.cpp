...
void SDR::Stream::SharedData::DirectX11Data::Create(int width, int height, bool sampling)
{
	...
	*/
	...
	if (sampling)
	{
		SDR::D3D11::OpenShader(Device.Get(), "Sampling", CSBlob_Sampling, sizeof(CSBlob_Sampling), SamplingShader.GetAddressOf());
		SDR::D3D11::OpenShader(Device.Get(), "ClearUAV", CSBlob_ClearUAV, sizeof(CSBlob_ClearUAV), ClearShader.GetAddressOf());
	}

	else
	{
		SDR::D3D11::OpenShader(Device.Get(), "PassUAV", CSBlob_PassUAV, sizeof(CSBlob_PassUAV), PassShader.GetAddressOf());
	}
}
...
void SDR::Stream::StreamBase::DirectX11Data::Create(ID3D11Device* device, HANDLE dx9handle, AVFrame* reference, bool staging)
{
	...
	SDR::D3D11::OpenShader(device, found->ShaderName, found->Data, found->DataSize, ConversionShader.GetAddressOf());
